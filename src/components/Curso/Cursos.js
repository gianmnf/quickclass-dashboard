...
const Cursos = () => {
        ...
        try {
            ...
            .then(querySnapshot => {
               ...
               if(result.length === 0) {
                    ...
                    swal("Erro", `Já existe um curso com este nome!`, "error");
                    setCurrentId('');
                }
            });
        } catch(err) {
    ...
    const addOrEdit = obj=>{
        ...
        try {
                ...
                .then(() => {
                    setCurrentId('');
                    swal("Sucesso", `Curso ${obj.nome} adicionado com sucesso!`, "success");
            });
        }
        catch(err) {
            swal("Erro", `${err}`, "error");
        ...
        try {
            app.firestore().collection('cursos').doc(cursoObjects[currentId].key).update({
                ...
                .then(() => {
                    setCurrentId('');
                    swal("Sucesso", `Curso ${obj.nome} atualizado com sucesso!`, "success");
                });
        }
        catch(err) {
            swal("Erro", `${err}`, "error");
    ...
    const onDelete = key=>{
            try {
                swal({
                    title: "Tem certeza de que deseja remover este curso?",
                  ...
                  .then((willDelete) => {
                    if (willDelete) {
                        ...
                        .then(() => {
                            setCurrentId('');
                            swal("Curso removido com sucesso!", {
                                icon: "success",
                              });
                        });
                    } else {
